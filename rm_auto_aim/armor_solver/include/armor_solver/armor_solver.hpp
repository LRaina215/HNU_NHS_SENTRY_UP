...
private:
  ...
  // 找到 selectBestArmor 的声明，改回原版签名（带 target_center）
  // int selectBestArmor(const std::vector<Eigen::Vector3d> &armor_positions,
  //                     const Eigen::Vector3d &target_center, // <--- 加回这个参数
  //                     const double target_yaw,
  //                     const double target_v_yaw,
  //                     const size_t armors_num) const noexcept;
  ...
  // 0202
  double anti_spin_bias_;
  double spin_judge_low_thres_;
  double fire_permit_uncertainty_;

  // 插值法pitch补偿参数
  std::vector<std::pair<double, double>> pitch_compensation_points_;
