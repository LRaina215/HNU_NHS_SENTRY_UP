...
: Node("armor_detector", options)
{
  ...
  enemy_color_sub = this->create_subscription<std_msgs::msg::Int32>(
    "red_blue_info", 10,
    std::bind(&ArmorDetectorNode::enemy_color_callback, this, std::placeholders::_1));
  // Detector
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr img_msg)
{
  ...
  try {
    ...
    FYT_ERROR("armor_detector", "Unknown Error in lookUpTransform");
    return;
  }
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  if (debug_) {
    ...
    cv::putText(
      img, latency_s, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX,
